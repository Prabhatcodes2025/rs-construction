...
function Dashboard({ initial, counts, leads, quickAdd, viewLeads }: { initial: { storage: StorageState }; counts: Record<string, number>; leads: Item[]; quickAdd: (module: string) => void; viewLeads: () => void }) {
  ...
  return <>
    ...
    <div className="admin-dashboard-grid">
      ...
      <div className="admin-panel"><h2>Lead sources</h2><div className="source-bars">{["RS Interiors Scope Planner", "Enquiry Popup", "Contact Form", "Package", "Project"].map(name => <div key={name}><span>{name}</span><i><b style={{ width: `${Math.min(100, Math.max(8, leads.filter(item => valueOf(item.source).includes(name)).length * 20))}%` }} /></i></div>)}</div></div>
...
function Enquiries({ leads, allLeads, search, status, source, setSearch, setStatus, setSource, view, remove, update }: { leads: Item[]; allLeads: Item[]; search: string; status: string; source: string; setSearch: (value: string) => void; setStatus: (value: string) => void; setSource: (value: string) => void; view: (lead: Item) => void; remove: (lead: Item) => void; update: (lead: Item, key: string, value: string) => void }) {
  return <div className="admin-panel"><div className="panel-title"><div><span>Lead management</span><h1>Enquiries</h1><p>{allLeads.length} total enquiries with status and internal notes.</p></div><a href="/api/admin/export"><Download />Export CSV</a></div>
    <div className="admin-filters"><label><Search /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search name, phone, location..." /></label><select value={source} onChange={event => setSource(event.target.value)}><option>All</option><option>Interiors Scope Planner</option><option>Popup</option><option>Contact Form</option><option>Package</option><option>Project</option></select><select value={status} onChange={event => setStatus(event.target.value)}><option>All</option><option>New</option><option>Contacted</option><option>Converted</option><option>Closed</option></select></div>
...
function LeadModal({ lead, close, update }: { lead: Item; close: () => void; update: (key: string, value: string) => void }) {
  const plannerDetails = lead.details && typeof lead.details === "object" ? Object.entries(lead.details as Record<string, unknown>) : [];
  const plannerLabels: Record<string, string> = { propertyType: "Property type", bedrooms: "Number of bedrooms", kitchenType: "Kitchen type", interiorPackage: "Interior package", requiredSpaces: "Required spaces", propertyLocation: "Property location", carpetArea: "Carpet area", additionalMessage: "Additional message" };
  return <div className="admin-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><div className="admin-modal lead-modal"><header><div><span>Enquiry details</span><h2>{valueOf(lead.name)}</h2></div><button onClick={close}><X /></button></header><div className="admin-modal-body"><div className="lead-detail-grid">{[["Phone", lead.mobile], ["Email", lead.email], ["Plot location", lead.location], ["Plot size", lead.plotSize], ["Source", lead.source], ["Date", formatDate(lead.createdAt, true)], ...plannerDetails.map(([key, value]) => [plannerLabels[key] || key, key === "carpetArea" && value ? `${value} sq.ft` : value]), ["Message", lead.message]].map(([label, value]) => <div key={String(label)}><span>{String(label)}</span><strong>{valueOf(value) || "Not provided"}</strong></div>)}</div><label className="admin-field"><span>Status</span><select value={valueOf(lead.status)} onChange={event => update("status", event.target.value)}><option>New</option><option>Contacted</option><option>Converted</option><option>Closed</option></select></label><label className="admin-field"><span>Internal notes</span><textarea rows={5} value={valueOf(lead.notes)} onChange={event => update("notes", event.target.value)} placeholder="Add follow-up notes for the team..." /></label></div><footer><button className="modal-save" onClick={close}>Done</button></footer></div></div>;
}
